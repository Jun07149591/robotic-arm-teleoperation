...
坐标映射:
  VR 空间:   +X 右,    +Y 上,  -Z 前
  Robot 空间: +X 右,    -Y 前,  +Z 上

  Robot_X = +VR_X      (VR 右 → Robot 右)
...
class ROSSimSender:
    """向 ROS 2 仿真环境发送 JointTrajectory 命令 (含二阶滤波 + 速度外推)"""

    def __init__(self, topic: str = "/arm_controller/joint_trajectory",
                 filter_omega: float = 14.0, traj_dt: float = 0.08):
        ...
        self._q_raw = None          # IK 原始输出
        self._q_filter_pos = None   # 滤波后位置
        self._q_filter_vel = None   # 滤波后速度
        self._dirty = False
        self._running = False
        self._dt = 0.03  # ~30Hz
    ...
    def start(self):
        ...
        self._thread.start()
        print(f"[ROS-Sim] 发送 JointTrajectory → /arm_controller (filter={self._filter_omega}Hz, dt={self._traj_dt}s)")
    ...
    def _apply_filter(self):
        """二阶低通滤波 (同 Xbox teleop _send_filtered)"""
        if self._q_filter_pos is None:
            self._q_filter_pos = list(self._q_raw)
            self._q_filter_vel = [0.0] * 6
            return
        ...
        ea = math.exp(-a)
        for i in range(6):
            err = self._q_raw[i] - self._q_filter_pos[i]
            vel = self._q_filter_vel[i]
            err_new = ea * ((1.0 + a) * err - dt * vel)
            vel_new = ea * (omega * omega * dt * err + (1.0 - a) * vel)
            self._q_filter_pos[i] = self._q_raw[i] - err_new
            self._q_filter_vel[i] = vel_new
        ...
        while self._running and rclpy.ok():
            with self._lock:
                if not self._dirty:
                    time.sleep(0.01)
                    continue
                q_raw = list(self._q_raw)
                self._dirty = False

            self._apply_filter()

            # 速度外推 (同 Xbox teleop, 防止 spline 振荡)
            extrapolated = list(self._q_filter_pos)
            if self._q_filter_vel is not None:
                for i in range(6):
                    extrapolated[i] += self._q_filter_vel[i] * self._traj_dt

            msg = JointTrajectory()
            msg.joint_names = JOINT_NAMES_ROS
            point = JointTrajectoryPoint()
            point.positions = extrapolated
            point.velocities = list(self._q_filter_vel) if self._q_filter_vel else [0.0] * 6
...
class VRTeleopSim:
    ...
    def __init__(self, scale_pos=1.0, scale_rot=1.0, dead_zone=0.02,
        ...
        # 居中工作姿态 (同 Xbox teleop HOME_POSITIONS), 远离 workspace 边界
        self._q_default = [
            0.0,                # L1: 中立
            math.radians(45),   # L2: 前倾 45°
            math.radians(-45),  # L3: 弯曲 45°
            0.0,                # L4: 中立
            0.0,                # L5: 中立
            0.0,                # L6: 中立
        ]
    ...
    def update(self, packet: dict) -> Optional[TransformDebug]:
        ...
        if self._smooth_delta_pos is None:
        ...
        try:
            q_new = kin.inverse_kinematics(
                target_pose, q_init=self._q_current,
                max_iter=200, damping=1e-4, dt=0.1,
            )
        except Exception as e:
...
async def run_real(args):
    ...
    # 读取真实机械臂当前关节角作为初始值，防止跳动
    q_real = arm.GetArmJointMsgs().to_list(include_gripper=False)
    teleop._q_current = q_real[:6]
    teleop._q_default = q_real[:6]
    print(f"[Arm] 当前真实关节角作为初始值: {[f'{math.degrees(v):+.1f}°' for v in teleop._q_current]}")
...
def parse_args():
    ...
    # 遥操作参数
    p.add_argument("--scale-pos", type=float, default=0.1,
                   help="位置缩放 (VR m → 机械臂 m), 默认 0.1 (VR 10cm → Robot 1cm)")
    p.add_argument("--scale-rot", type=float, default=0.0,
                   help="姿态缩放, 默认 0.0 (先固定姿态验证位置方向)")
