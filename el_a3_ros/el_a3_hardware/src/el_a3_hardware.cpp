  ...
  const rclcpp_lifecycle::State& /*previous_state*/)
{
  ...
  for (int retry = 0; retry < 20 && !all_valid; ++retry) {
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    all_valid = true;
    for (size_t i = 0; i < joint_configs_.size(); ++i) {
      const auto& config = joint_configs_[i];
      auto fb = can_driver_->getMotorFeedback(config.motor_id);
      if (fb.is_valid) {
        double joint_pos = (fb.position - config.position_offset) * config.direction;
        initial_positions[i] = joint_pos;
      } else {
        all_valid = false;
      }
    }
